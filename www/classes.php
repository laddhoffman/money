...
class Finances implements JsonSerializable {
    ...
    function get_today($print_tsv) {
        ...
        foreach ($this->accounts->list as $name => $account) {
            $balance = $account->get_balance();
            $results['each_account'][$name] = $balance;
        }
...
class MoneyItem implements JsonSerializable {
    ...
    function add_amount($amount, $date_start, $date_end, $period, $extra) {
        ...
        if ($debug_equity) {
            echo "echo add_amount: extra=$extra\n";
        }
        $this->list[] = $item;
        return $item;
    }
...
class Loan extends MoneyItem implements JsonSerializable {
    ...
    // a loan account can be set up to have a recurring deposit from a checking account
    ...
    function make_payment($date) {
        global $debug_equity;
        $this_checking = $this->checking;
        $default_checking = $this->finances->default_checking;
        $amount = $this->get_amount($date);
        if ($amount == 0) {
            return $amount;
        }
        if ($amount < 0) {
            $checking = $this_checking;
        } else {
            $checking = $default_checking;
        }
        if ($debug_equity) { echo "make_payment, $date, $this->checking_name, $amount, $this->balance\n"; }
        if ($this->balance < $amount) {
            $amount = $this->balance;
        }
        if ($checking->balance < $amount) {
            throw new Exception("Cannot make loan payment! $amount > $checking->balance");
        }
...
class Holding extends Account implements JsonSerializable {
    ...
    function set_checking_name($finances, $checking_name) {
        $this->earning->set_checking_name($finances, $checking_name);
    }
    ...
    function compute_transfers($date) {
        ...
        if (isset($this->earning->checking)) {
            $checking = $this->earning->checking;
        } else {
            throw new Exception("Holding: no checking acnt set");
        }
